...
class Enemy(GridObject):
    def __init__(self, game, x, y, enemy_type="basic_enemy", modifiers=None):
        ...
        self.damage = damage
        self.base_damage = damage # Store base damage for restoration
    ...
    def update(self, target_pos_or_flow_field, entities=None):
        ...
        if self.kamikaze_mode:
            ...
            # Restore Damage if we were in Kamikaze mode (and exited somehow)
            ...
            if hasattr(target_pos_or_flow_field, 'get_vector'):
                ...
                # Healer Isolation/Kamikaze Check
                    ...
                    if entities:
                        for entity in entities:
                            if entity is not self and isinstance(entity, Enemy) and entity.health > 0:
                                ...
                                # Let's assume anyone with behavior != healer is a protector
                                if entity.behavior_name != "healer": 
            ...
            if current_time - self.channeled_check_timer > 1000:
                ...
                if random.random() < 0.15:
                    ...
                    if best_target:
                        ...
                        self.channeled_last_tick = current_time
                        # debug.log(f"{self.enemy_type} started Channeled Heal on {best_target.enemy_type}")
                ...
                if entities:
                    ...
                    if closest_ally:
                        ...
                        if healed_amt > 0:
                            ...
                            # debug.log(f"Healer healed for {healed_amt}. Ally HP: {closest_ally.health}")
                            self.heal_cooldown_timer = current_time
                            self.last_successful_heal_time = current_time # Reset timer
